      ...
      res
        ...
        .json(
          apikeys.totalApiKeys > 0
            ? { apiKeys: apikeys.apiKeys, count: apikeys.totalApiKeys }
            : { message: 'No results founded!' }
        )
      return
    } catch (error) {
      return next(error)
  ...
  static async showSingleApiKey(
    ...
    try {
      ...
      res
        .status(StatusCodes.OK)
        .json(
          apikey ? { apikey } : { message: 'The related record is no founded' }
        )
      return
    } catch (error) {
      return next(error)
