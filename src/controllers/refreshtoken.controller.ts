    ...
    try {
      ...
      if (!refreshToken) {
        res
          ...
          .json({
            message:
              'Unauthorized: The access is restricted only for authorized users'
          })
        return
      }
      ...
      if (!owner) {
        res
          ...
          .json({ code: 403, message: 'Attempt to use wrong crendentials' })
        return
      }
      const accessTokenRefreshed =
        await RerfreshTokenService.signAccessToken(refreshToken)
