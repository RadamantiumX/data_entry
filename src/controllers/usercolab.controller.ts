...
export class UserColabController {
  ...
  static async createUserColab(
    ...
    try {
      ...
      res
        ...
        .json({ message: 'Success on create user', refreshToken })
      return
    } catch (error) {
      return next(error)
    }
  ...
  static async getAllUserColab(
    ...
    try {
      const allRecords: UserColabClientResponse =
        await UserColabService.getAllUserColab() // Prisma query function

      // Ternary
      res
        .status(StatusCodes.OK)
        .json(
          allRecords.totalUsers > 0
            ? { users: allRecords.users, count: allRecords.totalUsers }
  ...
  static async getUserColab(
    ...
    try {
      ...
      res
        .status(StatusCodes.OK)
        .json(userColab ? { user: userColab } : { message: 'No user found' })
      return
    } catch (error) {
      return next(error)
    }
  ...
  static async updateUserColab(
    ...
    try {
      ...
      res
        .status(StatusCodes.OK)
        .json({ message: `User ${req.body.username} updated` })
      return
    } catch (error) {
      return next(error)
    }
