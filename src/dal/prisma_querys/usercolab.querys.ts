...
export class UserColabQuerys {
  ...
  static async createRecord(
    payload: Pick<UserColab, 'username' | 'password' | 'isSuperAdmin'>
  ): Promise<void> {
  ...
  static async readRecord(
    payload: Pick<UserColab, 'id'>
  ): Promise<Omit<UserColab, 'password'> | null> {
  ...
  static async updateRecord(
    payload: Pick<UserColab, 'id' | 'username' | 'password' | 'isSuperAdmin'>
  ): Promise<void> {
   ...
   */
  static async setPayload(
    userColabId: string
  ): Promise<Omit<IPayload, 'currentDate'> | null> {
    const tokenPayload = await prisma.userColab.findUnique({
      where: { id: userColabId },
