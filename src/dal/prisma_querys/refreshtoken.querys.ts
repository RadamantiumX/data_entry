...
export class RefreshTokenQuerys {
  ...
  static async checkUserColab(
    userColbaId: string
  ): Promise<Pick<AuthRefreshToken, 'id'> | null> {
    const userColabRecord = await prisma.authRefreshToken.findUnique({
      where: { userColabId: userColbaId },
      select: { id: true }
    })
  ...
  static async checkingRecord(
    userId: string,
    refToken: string
  ): Promise<Pick<AuthRefreshToken, 'userColabId'> | null> {
    const checkToken = await prisma.authRefreshToken.findUnique({
      where: { userColabId: userId, refreshToken: { hasEvery: [refToken] } },
      select: { userColabId: true }
    })
    return checkToken
  }

  static async pushValueOnArrayRecord(
    userId: string,
    refToken: string
  ): Promise<void> {
  ...
  static async clearTokenValue(
    ...
    const arrayUpdated = uniqueRecord?.refreshToken.filter(
