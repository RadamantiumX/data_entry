...
export class AuthQuerys {
  ...
  static async uniqueRecord(
    username: Pick<UserColab, 'username'> | string | any
  ): Promise<Pick<
    UserColab,
    'id' | 'username' | 'password' | 'isSuperAdmin'
  > | null> {
    const user = await prisma.userColab.findUnique({
   ...
   */
  static async updateTimeStampSignInRecord(
    username: Pick<UserColab, 'username'>
  ): Promise<void> {
