...
export class ApiDataQuerys {
  ...
  static async createRecord(
    payload: Pick<ApiData, 'appName' | 'appId' | 'dataId'>
  ): Promise<void> {
  ...
  static async readCountRecords(): Promise<ApiDataClientResponse | null> {
    const [apiDatas, totalApiData] = await prisma.$transaction([
      prisma.apiData.findMany({
        select: { appName: true, appId: true, dataId: true },
  ...
  static async updateRecord(
    payload: Pick<ApiData, 'id' | 'appName' | 'appId' | 'dataId'>
  ): Promise<void> {
